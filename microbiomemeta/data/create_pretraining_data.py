...
def parse_args():
    ...
    parser.add_argument(
        "--input_file", help="Input raw text file (or comma-separated list of files)."
    )
    parser.add_argument(
        "--output_file",
    ...
    parser.add_argument(
        "--vocab_file", help="The vocabulary file that the ALBERT model was trained on."
    )
    parser.add_argument(
        "--spm_model_file",
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default="r",
        help="The data format of the input file.",
    )
    parser.add_argument(
        "--do_lower_case",
        action="store_true",
        default="Whether to lower case the input text. Should be True for uncased "
    ...
    parser.add_argument(
        "--do_permutation",
        action="store_true",
        help="Whether to do the permutation training.",
    )
    parser.add_argument(
        "--favor_shorter_ngram",
        action="store_true",
    ...
    parser.add_argument(
        "--ngram", type=int, default=3, help="Maximum number of ngrams to mask."
    )
    parser.add_argument(
...
def write_instance_to_example_files(
    instances, tokenizer, max_seq_length, max_predictions_per_seq, output_files, FLAGS
):
...
def is_start_piece(piece, FLAGS):
    if FLAGS.spm_model_file:
...
def create_masked_lm_predictions(
    tokens, masked_lm_prob, max_predictions_per_seq, vocab_words, rng, FLAGS
):
    ...
    for (i, token) in enumerate(tokens):
        ...
        if (
            ...
            and not is_start_piece(token, FLAGS)
        ):
            cand_indexes[-1].append(i)
        else:
            cand_indexes.append([i])
            if is_start_piece(token, FLAGS):
                token_boundary[i] = 1
