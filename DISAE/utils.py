...
def load_mtl_edges_from_file(edgefile, allowed_uniprots=None, sep=",", header=True):
    ...
    with open(edgefile, "r") as f:
        ...
        for line in f:
            ...
            count_loaded += 1
    logging.info(
        "{} edges loaded. {} edges (not-allowed-uniprots) skipped from {}".format(
            count_loaded, count_skipped, edgefile
        )
    )
    ikeys = list(set(ikeys))
...
def load_edges_from_file(edgefile, sep=",", header=True):
    # default data format:
    # chemical compound ID,protein ID,activity (sep=',')
    # e.g. MAEHEIXUINDDHE-UHFFFAOYSA-N,P48736,6.130
    ...
    chem_ids = list(set(chem_ids))
    prot_ids = list(set(prot_ids))
    return edges, chem_ids, prot_ids
...
def load_uniprot2pssm(max_len=512, padding=0):
    ...
    manual_dict = {
        "P52333_JH1domain-catalytic": "P52333_Kin.Dom.2-C-terminal.dat",
    ...
    with open(protfile, "r") as f:
        for line in f:
            ...
            line = (
                line.replace("-nonphosphorylated", "")
                .replace("-phosphorylated", "")
                .replace("-autoinhibited", "")
            )
            ...
            if pssm.shape[0] > max_len:
                print(
                    "Sequence length for {0} ({1}) is greater than {2}. Truncated to {2}".format(
                        uniprot, pssm.shape[0], max_len
                    )
                )
                pssm = pssm[:max_len, :]
            else:
    ...
    for gpcr_pssm_file in os.listdir(os.path.join(base_path, "gpcr_pssm_uniref50")):
        ...
        with open(
            os.path.join(os.path.join(base_path, "gpcr_pssm_uniref50"), gpcr_pssm_file),
            "r",
        ) as f:
            for line in f:
                line = line.strip().lstrip().split()
...
def get_mutant_triplets(genesymbol, mutations=None):
                ...
                for j, aa in enumerate(aligned_seq_residues):
                    ...
                    if resnum == position:
                        if aa.lower() == from_aa:
                            ...
                            print(
                                "Residue mismtach for {}. {} found instead of {} at {}".format(
    ...
    for i in selected_positions:
        if i in list(range(0, lgapcount)) or i in list(
            range(aligned_len - tgapcount, aligned_len)
        ):
            # leading or trailing gap
            seq.append(padchar)
        else:
            seq.append(
                aligned_seq_mut[i].lower().replace(".", padchar).replace("-", gapchar)
            )
    sentence = " ".join(seq)
    words = []
