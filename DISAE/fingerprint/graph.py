...
class Node(object):
    ...
    def __lt__(self, other):
        return self.ntype < other.ntype or (
            self.ntype == other.ntype and self.ext_id < other.ext_id
        )
    ...
    def _add_neighbor(self, neighbors, new_neighbors):
        ...
        if isinstance(new_neighbors, collections.Iterable) and all(
            [isinstance(node, Node) for node in new_neighbors]
        ):
            neighbors.update(new_neighbors)
        else:
            raise ValueError(
                "`neighbors` has to be either a Node object \
...
class Molecule(Graph):
    ...
    def get_neighbor_idx_by_degree(self, ntype, neighbor_type, degree):
        node_idx = {
            node.ext_id: idx
            for idx, node in enumerate(self.get_node_list(neighbor_type))
        }
